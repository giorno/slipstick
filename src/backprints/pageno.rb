...
module Io::Creat::Slipstick::Backprints
  ...
  class PageNoBackprint < Backprint
    ...
    def render ( )
      ...
      line_style = { :fill => ( @filled ? @line_style[:stroke] : 'none' ), :"stroke-width" => ( @filled ? 'none' : @line_style[:"stroke-width"] ), :stroke => @line_style[:stroke] }
      text_style = { :"font-size" => fs_mm, :fill => ( @filled ? 'white' : @line_style[:stroke] ) }
      @img.rectangle( @x_mm - w_mm / 2, @y_mm - @h_mm, w_mm, @h_mm, fs_mm / ( @filled ? 16 : 2 ), @line_style.merge( line_style ) )
      @img._text( @x_mm, @y_mm - ( @filled ? 0.15 : 0.6 ) * fs_mm, @text, @text_style.merge( text_style ) )
    end
