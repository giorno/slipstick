  ...
  module Model
	...
	# corresponding to values 1 and 10 on C/D scales
	  ...
	  [ @dm.x_mm + @dm.w_l_mm + @dm.w_s_mm, @dm.x_mm + @dm.w_l_mm + @dm.w_s_mm + @dm.w_m_mm ].each do | x_mm |
            @img.line( x_mm, @dm.y_mm + @dm.h_mm - @dm.cs_mm + @dm.h_mm / 2 - 3, x_mm, @dm.y_mm + @dm.h_mm - @dm.cs_mm + ( ( @dm.h_mm - @dm.hs_mm ) / 2 ) - 4, @style_contours ) # cut
            @img.line( x_mm, @dm.y_mm + @dm.h_mm - @dm.cs_mm + @dm.h_mm / 2 + 3.5, x_mm, @dm.y_mm + @dm.h_mm - @dm.cs_mm + ( ( @dm.h_mm + @dm.hs_mm ) / 2 ) + 4, @style_contours ) # cut
